...
from kernel_tuner.cache.cache import Cache, write_cache_file
...
class TestCache:
    ...
    @pytest.fixture
    def assert_create__raises_ValidationError(self, cache_path, header):
        yield
    ...
    def test_create__invalid_device_name(self, header, assert_create__raises_ValidationError):
        header.device_name = 3

    def test_create__invalid_kernel_name(self, header, assert_create__raises_ValidationError):
    ...
    def test_create__invalid_objective(self, header, assert_create__raises_ValidationError):
        header.objective = 3.141592

    @pytest.fixture
    def cache_file(self, cache_path, cache_json):
        write_cache_file(cache_json, cache_path)
        yield cache_path
    ...
    def cache_line_read(self, cache) -> Cache.Line:
        return cache.lines.get_from_params(a=0, b=0, c=0)
    ...
    def test_lines_get(self, cache, cache_lines):
        assert cache.lines.get_from_params(a=0, b=0, c=0) == cache_lines["0,0,0"]

    def test_lines_get__default(self, cache):
        assert cache.lines.get("1,0,0", "Hi!") == "Hi!"
        assert cache.lines.get_from_params(a=1, b=0, c=0, default="DEF") == "DEF"

    def test_lines_get__multiple(self, cache):
        assert len(cache.lines.get_from_params(b=1, c=1)) == 0
        assert len(cache.lines.get_from_params(b=2)) == 0
        assert len(cache.lines.get_from_params(a=0)) == 3
    ...
    def test_lines_get__with_non_existing_param_key(self, cache):
        with pytest.raises(ValueError):
            cache.lines.get_from_params(d=0)
    ...
    def test_line_attributes(self, cache_line_read, now):
        ...
        assert cache_line_read.timestamp == str(now)
    ...
    def test_line_dict__json_and_non_json(self, cache: Cache, now):
        ...
        line = cache.lines.get_from_params(a=1, b=1, c=1)
    ...
    @pytest.fixture
    def assert_append_line__raises_ValidationError(self, cache, cache_line):
        yield
        with pytest.raises(ValidationError):
            cache.lines.append(**vars(cache_line))
    ...
    def test_line_append__with_invalid_time(self, full_cache_line, assert_append_line__raises_ValidationError):
        full_cache_line.time = "abc"

    def test_line_append__with_invalid_compile_time(self, full_cache_line, assert_append_line__raises_ValidationError):
        full_cache_line.compile_time = True

    def test_line_append__with_invalid_verificat_time(self, full_cache_line, assert_append_line__raises_ValidationError):
        full_cache_line.verification_time = None

    def test_line_append__with_invalid_benchmark_time(self, full_cache_line, assert_append_line__raises_ValidationError):
        full_cache_line.benchmark_time = []

    def test_line_append__with_invalid_strategy_time(self, full_cache_line, assert_append_line__raises_ValidationError):
        full_cache_line.strategy_time = {}

    def test_line_append__with_invalid_framework_time(self, full_cache_line, assert_append_line__raises_ValidationError):
        full_cache_line.framework_time = False

    def test_line_append__with_invalid_timestamp(self, full_cache_line, assert_append_line__raises_ValidationError):
        full_cache_line.timestamp = 88

    def test_line_append__with_invalid_times(self, full_cache_line, assert_append_line__raises_ValidationError):
