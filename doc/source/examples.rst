...
.. code-block:: c
    ...
    __global__ void convolvution_kernel(float *output, float *input, float *filter) {
...
``tile_size_y``,
(not shown in the kernel code above)
...
The above code tells the kernel_tuner to compute the grid x-dimension
by taking the output image width and dividing it by the value of
