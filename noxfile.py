...
venvbackend_values = (
    ...
    "mamba",
    "venv",
)  # from https://nox.thea.codes/en/stable/usage.html#changing-the-sessions-default-backend
...
def create_settings(session: Session) -> None:
    ...
    if arg_trigger or (noxenv_file_path.exists() and not settings_file_path.exists()):
        ...
        envdir = ""
        # conversion from old noxenv.txt
...
def check_development_environment(session: Session) -> None:
    ...
    if session.posargs:
        if "github-action" in session.posargs:
            session.log(
                "Skipping development environment check on the GitHub Actions runner, as this is always up to date."
            )
            return None
...
def tests(session: Session) -> None:
    ...
    if install_cuda:
        ...
        if " not found: " in session.run("pip", "show", "pycuda", external=True, silent=True, success_codes=[0, 1]):
            ...
            pycuda_version = session.run(
                "python",
                "-c",
                "import pycuda.driver as drv; drv.init(); print('.'.join(list(str(d) for d in drv.get_version())))",
                silent=True,
            )
            shortest_string, longest_string = (
                (pycuda_version, cuda_version)
                if len(pycuda_version) < len(cuda_version)
                else (cuda_version, pycuda_version)
            )
    ...
    poetry_env = Path(
        session.run_always("poetry", "env", "info", "--executable", silent=not verbose, external=True)
        .splitlines()[-1]
        .strip()
    ).resolve()
