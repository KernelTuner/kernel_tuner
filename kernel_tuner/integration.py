...
def create_device_targets(header_filename, results_filename, objective=("time", min)):
    ...
    for gpu_name, problem_sizes in data.items():
        ...
        targets[key] = best_config_params
    ...
    if_block = ""
    for gpu_name, params in targets:
        if_block += f"#elif TARGET_GPU == {gpu_name}\n\n"
        if_block += "\n".join([f"#define {k} {v}" for k,v in params])
        if_block += "\n"

    default_config = "\n".join([f"#define {k} {v}" for k,v in default_params])
