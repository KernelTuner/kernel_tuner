...
def cli_convert(ap_res: argparse.Namespace):
    ...
    convert(
        ap_res.infile,
        write_file=ap_res.output,
        target=ap_res.target,
...
def parse_args(args):
    ...
    sp = parser.add_subparsers(
        required=True, help="Possible subcommands: 'convert', 'delete-line', 'get-line' and 'inspect'."
    )
    ...
    convert.add_argument(
        "--allow-version-absence",
        dest="allow_version_absence",
        action="store_true",
    ...
    delete.add_argument(
        "--key", required=True, help="The (potential) key of the (potential) cacheline entry to delete."
    )
    delete.add_argument(
        "-o", "--out", "--output", help="The (optional) output file to write the updated cachefile to.", dest="output"
    )
