...
try:
    ...
    class ExactGPModel(gpytorch.models.ExactGP):
        def __init__(self, train_x, train_y, likelihood, cov_kernel_name: str, cov_kernel_lengthscale: float):
            ...
            elif cov_kernel_name == 'additive_rbf':
                self.covar_module = AdditiveKernel(
                    num_dims=train_x.shape[1], 
                    base_kernel='rbf'
                )
            elif cov_kernel_name == 'additive_matern':
                self.covar_module = AdditiveKernel(
                    num_dims=train_x.shape[1], 
                    base_kernel='matern'
                )
...
supported_cov_kernels = ['matern', 'matern_scalekernel', 'additive']
...
class BayesianOptimization:
    ...
    def initialize_model(self, take_initial_sample=True, train_hyperparams=True):
        ...
        # Configurable jitter for numerical stability
        self.jitter = self.get_hyperparam("jitter", 1e-4, type=float)

        # create the model
        # if self.likelihood_name == 'Gaussian':
        # # Add priors for better regularization
        #     self.likelihood = gpytorch.likelihoods.GaussianLikelihood(
        #         noise_prior=gpytorch.priors.GammaPrior(1.0, 1.0),
        #         noise_constraint=gpytorch.constraints.GreaterThan(self.min_std)
        #     )
        if self.likelihood_name == 'Gaussian':
            self.likelihood = gpytorch.likelihoods.GaussianLikelihood()
        ...
        # For very small datasets, use RBF kernel which is more stable
        n_train = self.train_x.shape[0]
        if n_train < 5 and self.cov_kernel_name == 'matern_scalekernel':
            # Fallback to simpler kernel for very small datasets
            self.covar_module = gpytorch.kernels.ScaleKernel(gpytorch.kernels.RBFKernel())
            self.model = ExactGPModel(self.train_x, self.train_y, self.likelihood, 'rbf', self.cov_kernel_lengthscale)
        else:
    ...
    def get_lhs_samples(self, random_offsets: np.ndarray) -> Tensor:
        ...
        # Ensure diversity among samples
        param_configs_indices = self._ensure_diverse_samples(param_configs_indices, min_distance=max(1, self.size // (n_samples * 4)))
    ...
    def _ensure_diverse_samples(self, indices: Tensor, min_distance: int = 2) -> Tensor:
        """Filter samples to maintain minimum index distance for better exploration."""
        if len(indices) <= 1:
            return indices
        
        sorted_indices = indices.sort().values
        keep = [sorted_indices[0].item()]
        
        for idx in sorted_indices[1:]:
            idx_val = idx.item()
            if all(abs(idx_val - k) >= min_distance for k in keep):
                keep.append(idx_val)
        
        return torch.tensor(keep, device=self.device, dtype=torch.int)
    ...
    def train_hyperparams(self, training_iter: int):
        ...
        early_stopping_patience = self.get_hyperparam("early_stopping_patience", 50, type=int)
        early_stopping_tol = self.get_hyperparam("early_stopping_tol", 1e-8, type=float)
        ...
        def closure():
            ...
            try:
                ...
                if hasattr(output, 'covariance_matrix'):
                    eye = torch.eye(output.covariance_matrix.size(-1), device=self.device, dtype=output.covariance_matrix.dtype)
                    output.covariance_matrix = output.covariance_matrix + self.jitter * eye
        ...
        for i in range(training_iter):
            try:
                ...
                if loss < best_loss - early_stopping_tol:
                    best_loss = loss
                    no_improve = 0
                else:
                    no_improve += 1
                    if no_improve >= early_stopping_patience:
                        if self.tuning_options.verbose:
                            print(f"Early stopping at iteration {i}, best loss: {best_loss:.4f}")
                        break
    ...
    def optimize(self, max_fevals: int) -> List[dict]:    #NOSONAR
        ...
        # Configurable parameters for early stopping prevention
        exploration_prob = self.get_hyperparam("exploration_prob", 0.5, type=float)
        acq_restarts = self.get_hyperparam("acq_restarts", 5, type=int)
        convergence_window = self.get_hyperparam("convergence_window", 10, type=int)
        convergence_tol = self.get_hyperparam("convergence_tol", 1e-8, type=float)
        std_relative_var_threshold = self.get_hyperparam("std_relative_var_threshold", 1e-8, type=float)
        recent_optima = []
        ...
        try:
            ...
            while True:
                ...
                # Check if all STDs are effectively the same (using relative variance threshold)
                std_values = predictions_tuple[1]
                std_relative_var = std_values.std() / (std_values.mean() + 1e-8)
                std_all_same = std_relative_var < std_relative_var_threshold
                
                if mean_has_NaN or std_has_NaN or std_all_same:
                    least_evaluated_region_index = self.get_middle_index_of_least_evaluated_region()
                    param_config_index = least_evaluated_region_index
                    ...
                    if mean_has_NaN:
                        ...
                        warning_reason = f"all STDs were nearly the same (rel_var={std_relative_var:.2e})"
                    warnings.warn(
                    ...
                    # Epsilon-greedy exploration: occasionally pick random unvisited config
                    if torch.rand(1).item() < exploration_prob:
                        unvisited_indices = torch.where(self.unvisited_configs)[0]
                        if len(unvisited_indices) > 0:
                            short_param_config_index = torch.randint(0, len(unvisited_indices), (1,)).item()
                            param_config_index = unvisited_indices[short_param_config_index].item()
                        else:
                            # Fallback to acquisition optimization
                            param_config_index = self._optimize_acquisition(predictions_tuple, use_contextual_variance, acq_restarts, report_multiple_minima)
                    else:
                ...
                if result == self.invalid_value and short_param_config_index > -1:
                    ...
                    # Track convergence
                    recent_optima.append(self.current_optimum)
                    if len(recent_optima) > convergence_window:
                        recent_optima.pop(0)
                    
                    # Check convergence: stop if improvement stalls
                    if len(recent_optima) == convergence_window:
                        improvement = max(recent_optima) - min(recent_optima)
                        if improvement < convergence_tol:
                            ...
                            if self.tuning_options.verbose:
                                print(f"Converged after {self.fevals} evals (improvement: {improvement:.6f} < {convergence_tol})")
                            break
                if self.animate:
                    self.visualize()
    ...
    def _optimize_acquisition(self, predictions_tuple, use_contextual_variance, acq_restarts, report_multiple_minima):
        """Optimize acquisition function with random restarts to avoid local optima."""
        ...
        # Initial best from direct optimization
        short_param_config_index = self.argopt(acquisition_values).item()
        best_val = acquisition_values[short_param_config_index]
        best_idx = short_param_config_index
        
        # Random restarts: add noise and re-evaluate
        for _ in range(acq_restarts):
            noise = torch.randn_like(acquisition_values) * 0.1
            perturbed = acquisition_values + noise
            idx = self.argopt(perturbed).item()
            val = acquisition_values[idx]  # Evaluate original at perturbed optimum
            if self.is_better_than(val, best_val):
                best_val = val
                best_idx = idx
        
        # Convert short index (index in unvisited) to full index
        unvisited_indices = torch.where(self.unvisited_configs)[0]
        param_config_index = unvisited_indices[best_idx].item()

        # Handle multiple minima in acquisition function
        min_acquisition_function_value = acquisition_values[best_idx]
        ...
        if len(indices_where_min) > 1:
            ...
            if len(indices_where_min) > report_multiple_minima:
                warnings.warn(
                    f"After {self.fevals} fevals, there were multiple minima in the acquisition values ({len(indices_where_min)}), picking one based on the least evaluated region",
    ...
    def register_result(self, result: dict, param_config_index: int):
        ...
        if result is not None and last_result.get('time', self.invalid_value) != self.invalid_value and isinstance(last_result['time'], (int, float)):
...
class AdditiveKernel(gpytorch.kernels.Kernel):
    """Sum of 1D kernels; each parameter gets its own lengthscale."""
    def __init__(self, num_dims, base_kernel='rbf', **kwargs):
    ...
    def forward(self, x1, x2, diag=False, **params):
        ...
        for d in range(self.num_dims):
            k = self.base_kernels[d](x1[:, d:d+1], x2[:, d:d+1], diag=diag)
