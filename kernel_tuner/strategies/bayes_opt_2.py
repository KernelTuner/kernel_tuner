...
class BayesianOptimization():
    ...
    def draw_random_sample(self) -> Tuple[list, int]:
        """ Draw a random sample from the parameter configurations """
        index = randint(0, self.searchspace_size - 1)
        if self.is_not_visited(index):
            param_config = self.searchspace[index]
            return param_config, index
        return self.draw_random_sample()
    ...
    def initial_sample(self):
        ...
        _, std = self.predict_list(self.searchspace)
    ...
    def optimize(self, max_function_evaluations):
        ...
        while self.__visited_num < max_function_evaluations:
            ...
            predictions, std = self.predict_list(self.searchspace)
            ...
            if self.af_num == 1:
                candidate_index = self.argopt(list_of_acquisition_values)
                candidate_params = self.searchspace[candidate_index]
                ...
                raise ValueError("self.af_num != 1")
    ...
    def optimize_multi(self, max_function_evaluations):
        ...
        while self.__visited_num < max_function_evaluations:
            ...
            for af in aqfs:
                ...
                candidate_index = self.argopt(list_of_acquisition_values)
                del predictions[candidate_index]    # to avoid going out of bounds
                candidate_params = self.searchspace[candidate_index]
                observation = self.evaluate_objective_function(candidate_params)
                self.update_after_evaluation(observation, candidate_index, candidate_params)
            self.fit_observations_to_model()
    ...
    def af_random(self, predictions=None, hyperparam=None) -> list:
        ...
        list_random = range(len(self.searchspace))
        shuffle(list_random)
        return list_random
    ...
    def af_probability_of_improvement(self, predictions=None, hyperparam=None) -> list:
        ...
        if predictions is None:
            predictions, _ = self.predict_list(self.searchspace)
    ...
    def af_thompson_sampling(self, predictions=None, hyperparam=None) -> list:
        ...
        random_indices = randsample(range(0, self.searchspace_size - 1), 400)
        random_samples = [self.searchspace[i] for i in random_indices]
